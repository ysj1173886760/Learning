...
class ConvolutionalLayer(object):
    ...
    def backward_speedup(self, top_diff):
        ...
        backward_col = np.empty((self.input.shape[0], height_out * width_out, self.kernel_size * self.kernel_size * self.channel_out))
        cur = 0
        for x in range(height_out):
            for y in range(width_out):
                bias_x = x * self.stride
                bias_y = y * self.stride
                backward_col[:, cur, :] = top_diff[:, :, bias_x: bias_x + self.kernel_size, bias_y: bias_y + self.kernel_size].reshape(self.input.shape[0], -1)
                cur = cur + 1

        # backward_col [batch, height_out * width_out, k * k * cout]
        bottom_diff = np.matmul(backward_col, np.transpose(self.weight, [1, 2, 3, 0]).reshape(-1, self.channel_in))
        # [batch, height, width, cin]
        bottom_diff = np.transpose(bottom_diff, [0, 3, 1, 2])[:, :, self.padding: height_out - self.padding, self.padding: width_out - self.padding]
